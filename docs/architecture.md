...
### 7. API 健康状态监控
- **健康检查 API**: `/api/health` 端点检查各个 API 的健康状态
- **检查项**: GitHub API 连接、仓库树 API、环境变量配置
- **状态页面**: 独立的 `status.html` 页面显示详细的健康状态信息
- **实时监控**: 首页导航栏显示状态指示器，每 60 秒自动检查
- **状态类型**: 正常 (healthy)、降级 (degraded)、异常 (unhealthy/error)、未知 (unknown)
- **响应时间**: 显示每个 API 端点的响应时间，便于性能监控
